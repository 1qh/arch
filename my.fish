...
if type -q pacman
    ...
    set vs $repo/vscode
    abbr vscode "cp $vs/settings.json $df/Code/User"
    abbr apc sudo chown -R (whoami) /opt/visual-studio-code
end
...
if type -q xcode-select
    ...
    set dep _libgcc_mutex
end

if type -q $m
    if type -q nvidia-smi
        set py_nvidia -c nvidia pytorch-cuda
    end
    set pypkg $pkg/py
    set mb (cat $pypkg/mb.txt)
    set nodep (cat $pypkg/nodep.txt)
    set pip (cat $pypkg/pip.txt)
    set mim (cat $pypkg/mim.txt)
    set dj (cat $pypkg/dj.txt)
    set st (cat $pypkg/st.txt)

    set MAMBA_NO_PROMPT
    set mbroot ~/$m

    abbr mbempty $mrm $dep
    abbr mbbase --set-cursor "$mbi $py_nvidia% $mb"
    abbr mok --set-cursor "$mrm -y $dep && $mbi -y $py_nvidia $nodep % $mb && $m clean -ya"
    abbr mbnodep --set-cursor "$mbi $py_nvidia% $nodep"
    abbr mbrm1by1 --set-cursor "for i in (cat $pypkg/%); echo \n\n=== REMOVING \$i ===\n; $mrm \$i; end"
    abbr mbin1by1 --set-cursor "for i in (cat $pypkg/%); echo \n\n=== INSTALLING \$i ===\n; $mbi \$i; end"

    abbr pipsync "$pipin $pip"
    abbr djsync "$pipin $dj"
    abbr stsync "$pipin $st"
    abbr mimsync "mim install $mim"
    abbr pip2conda --set-cursor "openlink 'https://anaconda.org/search?q=' $pypkg/%.txt"
end
...
function fish_greeting
    $m shell hook --shell fish --prefix $mbroot | source
    $mac $mbroot
    clear -x
    if type -q dysk
        dysk /
