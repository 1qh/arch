...
set mchannel -c pytorch -c conda-forge
set mbi $m install $mchannel
set mrm $m remove
set mac $m activate
...
abbr mrm $mrm
abbr mbempty $mrm _libgcc_mutex

set pipin $p install -U
abbr pipin $pipin
abbr piprm $p uninstall
abbr pipall "$p freeze | rg ="
abbr pipempty "$p uninstall ($p freeze | rg =)"

abbr svshad --set-cursor 'set cn bunx shadcn-svelte && set d % && bun c svelte $d && cd $d && bunx svelte-add tailwindcss && bun i && $cn init -y && $cn add -a && $cn update -a && bun update'
abbr nextshad --set-cursor 'set cn bunx shadcn-ui && set d % && bunx create-next-app $d --ts --tailwind --eslint --app && cd $d && $cn init -y && $cn add -yo && bun update'
...
abbr mbcheck --set-cursor "set -l p %; $m repoquery depends $mchannel \$p | rg \$p; $mbi \$p"
...
abbr gitfresh 'git checkout --orphan z && git add -A && git commit -am . && git branch -D master && git branch -m master && git push -f origin master'
...
function _done_noti_ -e fish_postexec
    ...
    if not contains $base_cmd $excludes && test $CMD_DURATION -gt 10000
