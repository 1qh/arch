...
set rainbow lolcrab
abbr cf "code (fd -E $m/ -E runs/ -t d | sk)"
...
set mrm $m remove
set mac $m activate
set mbempty "mkdir -p ~/$m && $mac && $mbi -y python_abi && $mrm -ya"
...
abbr mbfish $m shell init -s fish
abbr mrm $mrm

set p uv pip
set uvin $p install -U
...
abbr mball0 "$m env export --from-history -n base | rg - | rg -v 'conda-forge' | sd '  - ' '' | nl"
...
function _done_noti_ -e fish_postexec
    set excludes bat cat cd code dua gitui man micro navi nvim sk vi vim yazi zellij
